...
class AiService(
    ...
    companion object {
        private const val MAX_AI_RESPONSE_LENGTH = 255
        private const val RESPONSE_TIMEOUT_SECONDS: Long = 60
        private const val CONNECT_TIMEOUT_MILLS = 10000
    }

    private val httpClient =
        HttpClient.create()
            .option(ChannelOption.CONNECT_TIMEOUT_MILLIS, CONNECT_TIMEOUT_MILLS)
            .doOnConnected { conn ->
                conn.addHandlerLast(ReadTimeoutHandler(RESPONSE_TIMEOUT_SECONDS, TimeUnit.SECONDS))
            }
    ...
    private fun systemMessage(username: String): String {
        return """
            Ты — профессиональный психолог с глубокими знаниями в когнитивно-поведенческой терапии,
            гештальт-подходе и психоанализе. Твоя задача — анализировать личные записи пользователя,
            выявлять эмоциональные паттерны, когнитивные искажения, внутренние конфликты, а также давать
            чёткую и обоснованную обратную связь.

            Обращай внимание на формулировки, выбирай точные термины (например: деперсонализация,
            тревожность, эмоциональное выгорание, защитные механизмы, рационализация, прокрастинация).

            Используй метафоры, аналогии, конкретные примеры и терапевтические вопросы. Стимулируй
            глубокую саморефлексию. Показывай, как определённые мысли и убеждения влияют на чувства и
            поведение.

            Не ограничивайся поддержкой — давай практические рекомендации, техники (например: дыхательные
            упражнения, ведение журнала мыслей, метод Сократа, "письмо внутреннему ребёнку").

            Будь внимательным и тактичным, но не уходи от сути — прямо обозначай внутренние противоречия,
            возможные ошибки мышления и избегания.

            Имя пользователя: "$username". Используй это имя точно в таком виде — **без перевода,
            склонения или транслитерации**. Только "$username".
            """.trimIndent()
    }
                        ...
                        AiMessageDto(
                            content =
                                "Скажи ровно одно самое подходящее название для этой заметки, " +
                                    "длиной не больше 30 символов. Не надо примеров, " +
